 ...
 * @author hp
 */
public class usersovertimechart extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            crs.setCommand("select ts from users order by ts");
            crs.execute();
            ...
            try {
                ...
                String date = null;
                int userCounter = 1;
                int cumulativeUserCounter = 0;
                r.append("[[\"Date\",\"New Users\", \"Cumulative\"],");
                for (Timestamp t : res) {
                    date = DateTimeConvertor.timeStampToDateForChart(t, "dd-MM-yy");
                    cumulativeUserCounter++;
                    if (date.equals(initDate)) {
                        userCounter++;
                    } else {
                        r.append("[\"" + initDate + "\"," + userCounter + "," + cumulativeUserCounter + "],");
                        initDate = date;
                        userCounter = 1;
                    }
                }
                ...
                if (date == null)
                    ...
                    r.append("[\"" + date + "\"," + userCounter + "," + cumulativeUserCounter + "]");
                r.append("]");
            } catch (Exception e) {
